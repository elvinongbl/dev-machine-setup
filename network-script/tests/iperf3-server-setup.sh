...
function stmmac_setup () {
	...
	local IFIPADDR=$STMMAC_SERVER_IP
...
echo -e "iperf server setup = stmmac [$STMMACIFNAME@$STMMAC_SERVER_IP]"
stmmac_setup

#echo -e "iperf server setup = igb [$IGBIFNAME@$IGB_SERVER_IP]"
#igb_setup
